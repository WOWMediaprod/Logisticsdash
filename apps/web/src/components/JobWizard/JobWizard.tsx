...
interface JobRequestFormData {
  ...
  clientId: string; // Used in admin mode
...
export default function JobWizard({
  ...
  const uploadFile = async (file: File, type: string): Promise<{id: string; fileUrl: string}> => {
    ...
    if (mode === 'client' && propsClientId) {
      const metadata = JSON.stringify({
        uploadedBy: 'client',
        clientId: propsClientId,
        clientName: propsClientName || 'Unknown'
      });
  ...
  const handleSubmit = async () => {
    if (!companyId) {
      alert('Missing company information');
      return;
    }

    // Validate clientId
    const effectiveClientId = mode === 'client' ? propsClientId : formData.clientId;
    ...
    try {
      ...
      if (releaseOrderDocId) {
        ...
        console.log('[JobWizard] Added release order document to supportingDocIds:', releaseOrderDocId);
      }
      ...
      console.log('[JobWizard] Total supporting documents:', supportingDocIds.length, supportingDocIds);
      ...
      // Submit to different endpoints based on mode
      const endpoint = mode === 'client' ? '/api/v1/job-requests' : '/api/v1/jobs';
      ...
      if (response.ok && result.success) {
        ...
        alert(`Failed to submit ${mode === 'client' ? 'request' : 'job'}: ${result.message || 'Unknown error'}`);
      }
    } catch (error) {
      ...
      <div className="container mx-auto px-4 py-16 max-w-2xl">
        ...
        >
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-4">
            {mode === 'client' ? 'Request Submitted!' : 'Job Created!'}
          </h1>
          <p className="text-gray-600 mb-2">
            {mode === 'client'
              ? 'Your job request has been submitted for review.'
              : 'Your job has been created successfully.'}
          </p>
          <p className="text-sm text-gray-500 mb-8">
            ID: <span className="font-mono font-semibold">{jobId}</span>
          </p>
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">
                {mode === 'client' ? 'New Job Request' : 'Create Job'}
              </h1>
              <p className="text-gray-600">
                {mode === 'client'
                  ? 'Submit a new logistics request for review'
                  : 'Create a new logistics job'}
              </p>
            </div>
            {mode === 'client' && propsClientName && (
              <div className="text-right">
                <p className="text-sm text-gray-500">Client</p>
                <p className="font-semibold text-gray-900">{propsClientName}</p>
                <p className="text-sm text-gray-500">{propsClientCode}</p>
              </div>
            )}
            {onCancel && (
              <button
...
function Step1BasicInfo({ formData, setFormData, onNext, mode, clients, loadingClients }: Step1Props) {
  const canProceed = formData.title && formData.shipmentType && (mode === 'client' || formData.clientId);
...
function Step5Review({ formData, onSubmit, onPrevious, submitting, onEdit, mode, clients }: Step5Props) {
  ...
  return (
    <motion.div
      ...
      className="space-y-6"
    >
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Review Your {mode === 'client' ? 'Request' : 'Job'}</h2>
      ...
      <div className="bg-gray-50 rounded-xl p-6">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          {mode === 'admin' && selectedClient && (
            <div>
              <span className="text-gray-600">Client:</span>
              <p className="font-medium">{selectedClient.name} ({selectedClient.code})</p>
            </div>
          )}
          <div>
        ...
        >
          {submitting ? (
            ...
            <>
              ...
              Submit {mode === 'client' ? 'Request' : 'Job'}
            </>
          )}
        </button>
      </div>
