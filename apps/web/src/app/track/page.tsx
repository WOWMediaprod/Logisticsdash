...
export default function DriverTrackingPage() {
  ...
  const sendLocationViaHTTP = async (locationData: LocationData) => {
    try {
      ...
      if (result.success) {
        console.log('✅ Location sent via HTTP successfully:', result);
      } else {
        console.error('❌ HTTP location update failed:', result.error);
      }
    } catch (error) {
      console.error('❌ Failed to send location via HTTP:', error);
    }
  };
  ...
  const startTracking = () => {
    ...
    if (!socket || !isConnected) {
      showStatus('⚠️ Starting offline tracking (WebSocket unavailable)', 'info');
    } else {
      showStatus('✅ Starting live tracking...', 'info');
    ...
    watchIdRef.current = navigator.geolocation.watchPosition(
      (position) => {
        ...
        // Send live driver location update to the logistics API if connected
        if (socket && isConnected) {
          socket.emit('live-driver-location', locationData);
        } else {
          // Fallback to HTTP when WebSocket is not available
          console.log('📍 Location captured, sending via HTTP:', locationData);
          sendLocationViaHTTP(locationData);
        }

        if (!isTracking) {
          setIsTracking(true);
          const statusMsg = (socket && isConnected)
            ? '✅ Live tracking active - Updates every 10 seconds'
            : '📡 HTTP tracking active - Sending location updates';
