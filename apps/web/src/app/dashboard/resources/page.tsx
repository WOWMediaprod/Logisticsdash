...
type TabType = 'containers' | 'vehicles' | 'trailers' | 'drivers' | 'clients' | 'companies';
...
export default function ResourcesPage() {
    ...
    try {
      ...
      switch (activeTab) {
        ...
        case 'vehicles':
          ...
          break;
        case 'trailers':
          endpoint = `/api/v1/trailers?companyId=${companyId}`;
          break;
        case 'drivers':
  ...
  const tabs = [
    ...
    { id: 'trailers' as TabType, label: 'Trailers', icon: Truck },
...
function TrailersTable({
  ...
  trailers: Trailer[];
  onEdit: (item: Trailer) => void;
  onDelete: (id: string) => void;
}) {
  if (trailers.length === 0) {
    return <div className="text-center py-8 text-gray-500">No trailers found. Click "Add trailer" to create one.</div>;
  }
    ...
    <table className="w-full">
      <thead>
        <tr className="border-b border-gray-200">
          ...
          <th className="text-left py-3 px-4">Axles</th>
          <th className="text-left py-3 px-4">Status</th>
          <th className="text-right py-3 px-4">Actions</th>
        </tr>
      </thead>
      <tbody>
        {trailers.map((trailer) => (
          <tr key={trailer.id} className="border-b border-gray-100 hover:bg-gray-50">
            <td className="py-3 px-4 font-medium">{trailer.regNo}</td>
            <td className="py-3 px-4">{trailer.type}</td>
            <td className="py-3 px-4">
              {trailer.make && trailer.model ? `${trailer.make} ${trailer.model}` : trailer.make || trailer.model || '-'}
            </td>
            <td className="py-3 px-4">{trailer.year || '-'}</td>
            <td className="py-3 px-4">{trailer.capacity || '-'}</td>
            <td className="py-3 px-4">{trailer.axles || '-'}</td>
            <td className="py-3 px-4">
              <span className={`px-2 py-1 rounded-full text-xs ${trailer.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {type === 'trailers' && <TrailerForm formData={formData} setFormData={setFormData} />}
...
function TrailerForm({ formData, setFormData }: { formData: any; setFormData: (data: any) => void }) {
  return (
    <>
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">Registration Number *</label>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Axles</label>
          <input
            type="number"
            value={formData.axles || ''}
            onChange={(e) => setFormData({ ...formData, axles: parseInt(e.target.value) || null })}
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
            placeholder="e.g., 3"
            min="1"
            max="10"
      ...
      <div>
        ...
        <input
          ...
          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
          placeholder="e.g., 40ft, 45000kg, 25 tons"
        />
      </div>
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Lease per Day</label>
          <input
            type="number"
            step="0.01"
            value={formData.leasePerDay || ''}
            onChange={(e) => setFormData({ ...formData, leasePerDay: parseFloat(e.target.value) || null })}
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
            placeholder="Daily lease cost"
          />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Maintenance per KM</label>
          <input
            type="number"
            step="0.0001"
            value={formData.maintPerKm || ''}
            onChange={(e) => setFormData({ ...formData, maintPerKm: parseFloat(e.target.value) || null })}
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
            placeholder="Maintenance cost per km"
          />
        </div>
      </div>
      <div className="flex items-center">
